...
class PaintClassicXP(tk.Tk):
    def __init__(self):
        ...
        # caminho do arquivo atualmente aberto (se houver)
    ...
    def open_image(self):
        ...
        try:
            ...
            # Use o tamanho nativo da imagem e atualize o widget Canvas para acompanhar
            self.image = img
            self.draw = ImageDraw.Draw(self.image)
            try:
            ...
            # atualizar título para indicar arquivo aberto
            try:
                self.title(f"Paint - {file_path}")
            except Exception:
                pass
    ...
    def _build_center_canvas(self):
        ...
        # Inicializa o widget Canvas com o tamanho atual do buffer de imagem
        self.canvas = tk.Canvas(center, bg="white", width=self.image.width, height=self.image.height, cursor="cross", highlightthickness=1, highlightbackground="#808080")
    ...
    def _update_canvas_image(self):
        ...
        # Garanta que a região do canvas corresponda ao tamanho da imagem
        try:
    ...
    def clear_all(self):
        # Limpa mantendo o tamanho atual do buffer/canvas para não alterar a interface
        w, h = self.image.width, self.image.height
        self.image = Image.new("RGBA", (w, h), BG_COLOR)
        self.draw = ImageDraw.Draw(self.image)
        try:
    ...
    def _resize_canvas_dialog(self):
        ...
        def apply():
            ...
            self.image = new_img
            self.draw = ImageDraw.Draw(self.image)
            # Atualiza o widget Canvas para acompanhar o novo tamanho
            try:
                self.canvas.config(width=w, height=h)
            except Exception:
                pass
